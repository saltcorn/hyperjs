// use http_body_util::BodyExt; // for the BodyExt trait
// use hyper::{Body, Request, Response, StatusCode};
// use std::convert::Infallible;

// async fn handle_request(req: Request<Body>) -> Result<Response<Body>, Infallible> {
//   // Only process POST or PUT requests with a body
//   if req.method() == hyper::Method::POST || req.method() == hyper::Method::PUT {
//     let body_bytes = match req.into_parts().1.collect().await {
//       Ok(collected) => collected.to_bytes(),
//       Err(_) => {
//         let mut res = Response::new(Body::from("Failed to read body"));
//         *res.status_mut() = StatusCode::INTERNAL_SERVER_ERROR;
//         return Ok(res);
//       }
//     };

//     // Deserialize the body into your struct
//     let parsed_data: Result<MyData, serde_json::Error> = serde_json::from_slice(&body_bytes);
...
//       }
//       Err(e) => {
//         let mut res = Response::new(Body::from(format!("Failed to parse JSON: {}", e)));
...
//     let mut res = Response::new(Body::from("Please send a POST request"));
